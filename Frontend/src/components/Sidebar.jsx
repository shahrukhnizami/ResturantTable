...
const Sidebar = ({ isOpen, setIsOpen }) => {
  ...
  // console.log(restaurantData, "ABC");
  ...
  useEffect(() => {
    ...
    if (matchedItem) {
      setActive(
        matchedItem.subItems?.find((sub) => sub.path === currentPath)?.label ||
          matchedItem.label
      );
  ...
  //   { icon: <SquareUserRound  className="w-6 h-6" />, label: 'Our Clients', path: '/admin/ourclients' },
  //   {
  //     icon: <Hotel className="w-6 h-6" />,
  //     label: 'Restaurants',
  //     subItems: restaurantData?.data?.map(({ restaurantName, _id }) => ({
  //       label: restaurantName,
  //       path: `/admin/restaurant/${restaurantName.toLowerCase().replace(/\s+/g, "-")}/${_id}`,
  //       value: _id,
  //     })),
  //   },
    ...
    ...(user?.user?.role === "super-admin"
      ? [
          ...
          // },
          {
            ...
            label: "Our Clients",
            path: "/admin/ourclients",
          },
          {
            icon: <Users className="w-6 h-6" />,
            label: "Resgister Admin",
            path: "/admin/admin-resturant",
          },
         
          {
            ...
            subItems: restaurantData?.data?.map(({ restaurantName, _id }) => ({
              label: restaurantName,
              path: `/admin/restaurant/${restaurantName
                .toLowerCase()
                .replace(/\s+/g, "-")}/${_id}`,
              value: _id,
            })),
      ...
      : [
          // { icon: <GitBranch className="w-6 h-6" />, label: "Add Branch", path: "/admin/add-branch" },
          {
            ...
            label: "Our Resturant",
            path: "/admin/resturant-detail",
          },
          // { icon: <User className="w-6 h-6" />, label: "Add Admin", path: "/admin/add-admin" },
        ]),
    {
      icon: <User className="w-6 h-6" />,
      label: "Profile",
  ...
  return (
    <aside style={{ color: colors.primary }} 
    ...
    >
      <div className="p-4 border-b  flex justify-between items-center">
        <div  className="flex items-center  gap-3">
          <Icons.Fullscreen style={{ color: colors.primary }} 
            className={`w-8 h-8 text-amber-400  ${isOpen ? "" : "mx-auto"}`}
          />
          {isOpen && (
            <div>
        ...
        <button 
         style={{ background: colors.primary }}
          ...
          className="text-white cursor-pointer bg-[#8ce8be] border-1 hover:text-white  rounded-full hover:bg-[#8ce8be]"
        >
          {isOpen ? (
            ...
            {item.subItems ? (
              <div>
                <button
                  ...
                  className={`w-full cursor-pointer flex items-center justify-between px-4 py-3 rounded-lg 
                      ${
                        active === item.label
                          ? "text-black bg-[#1E3A8A]"
                          : "text-black hover:bg-[#1E3A8A] hover:text-white"
                      } 
                  ...
                  {isOpen && (
                    <ChevronDown
                      className={`w-4 h-4 transform ${
                        isRestaurantSubOpen ? "rotate-180" : "rotate-0"
                      }`}
                    />
                  )}
                </button>
                  ...
                  <div className="ml-8 space-y-1 mt-2">
                    {item.subItems.map((subItem, subIndex) => (
                      <button
                        ...
                        className={`w-full cursor-pointer flex items-center px-4 py-2 rounded-lg text-left text-sm 
                            ${
              ...
              <button
                ...
                className={`w-full cursor-pointer flex items-center gap-3 px-4 py-3 rounded-lg 
                    ${
                      active === item.label
                        ? "text-indigo-800 shadow-md font-bold"
                        : "text-black hover:bg-indigo-900 hover:text-white"
                    } 
